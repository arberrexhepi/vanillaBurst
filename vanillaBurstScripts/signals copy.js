...
ë.frozenVanilla("vanillaSignal", async function (signal) {
  ...
  if (onEvent && Array.isArray(onEvent)) {
    ...
    let eventResult = await ë.one(onEvent[0], onEvent[1], true, signalObject);
      ...
      callBack: async (action, status, value) => {
        ...
        switch (action) {
          ...
          case "pause":
            ...
            break;
          case "reset":
            await ë.unsignalInterval("timeSignal", true);
            break;
          case "complete":
            await callBack(true, value);
        }
      },
      clear: clear,
...
ë.frozenVanilla(
  ...
  async function (
    ...
    } else if (clear && typeof clear === "function") {
      clear();
    }
  }
);
...
ë.frozenVanilla("unsignalInterval", function (signalName, remove) {
  return new Promise((resolve, reject) => {
    let storedIntervals = JSON.parse(localStorage.getItem("signals"));

    for (let name in storedIntervals) {
      if (name === signalName) {
        if (storedIntervals[name]?.clear === "clear") {
          // alert(storedIntervals[name].id);
          clearInterval(storedIntervals[name].id[name]);
          if (remove && remove === true) {
            delete storedIntervals[name];
            localStorage.setItem("signals", JSON.stringify(storedIntervals));
          }
          resolve(`Signal ${signalName} unregistered successfully.`);
        } else {
          reject(`Failed to unregister signal ${signalName}.`);
        }
      }
    }
    reject(`Signal ${signalName} not found.`);
  });
});
